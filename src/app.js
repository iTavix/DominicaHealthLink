...
const lucide = { createIcons: (opts) => createIcons({ icons: lucideIcons, ...(opts || {}) }) };
  ...
  // Old bureaucratic states 2-7 (docs, ministry, decree) fall under phase 2 (Gestione
  // Documentale); visa/OPI obtained (8-9) → phase 4 (ready to travel); arrival (10) → 5;
  // completed (11) → DONE_STEP.
  ...
  function setDocStatus(nurseId, docId, status) {
    const n = getNurse(nurseId); if (!n) return; // stale DOM: candidate removed by another operator
    const d = n.documents.find((x) => x.id === docId); if (!d || d.status === status) return;
    if (!canOperatePhase(n.currentStep)) return; // the other team's phase
